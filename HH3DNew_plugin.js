    ...
    try {
        ...
        $child.find("a").each(function() {
            ...
            if (blockHtml) {
                ...
                while ((aMatch = aTagRegex.exec(blockHtml)) !== null) {
                    var aTag = aMatch[0];
    ...
    try {
        ...
        allLink.forEach(function(item) {
            if (item.name.match(/4k/i) && item.link.indexOf('.m3u8') > -1) {
                ...
                pool.hd = item.link;
            } else if (item.link.indexOf('.m3u8') > -1) {
...
function buildMenu(listurl){var menulist=[];if (!listurl)return menulist;var lines=listurl.split('\n');for (var i=0;i < lines.length;i++){var line=lines[i].trim();if (!line||line.indexOf('@@')===-1)continue;var parts=line.split('@@');var link=parts[0]?parts[0].trim():"";var name=parts[1]?parts[1].trim():"";var check=parts[2]?parts[2].trim():undefined;if (!link||!name)continue;var item={};if (check==="false"){item={"slug":link,"title":name,"type":"Horizontal"};}else if (check==="true"){item={"slug":link,"title":name,"type":"Grid"};}else{item={"slug":link,"name":name};}menulist.push(item);}return menulist;}
