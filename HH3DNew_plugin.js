...
function getManifest() {
    return JSON.stringify({
        ...
        "version": "1.3.9", // Tăng nhẹ phiên bản để app làm mới cache
        "baseUrl": BASEURL,
        "iconUrl": "https://bilutv.asia/img/bilutvlogo-ngang.jpg",
        "isEnabled": true,
        "layoutType": "HORIZONTAL",
        "type": "MOVIE",
        "playerType": "exoplayer" // Giữ nguyên playerType gốc chạy siêu mượt trên iOS
    });
}
...
// Khai báo fallback dự phòng cho iOS tránh rò rỉ bộ nhớ
...
function parseMovieDetail(htmlContent, url) {
    try {
        ...
        $child.find("a").each(function() {
            ...
            // Kế thừa thuật toán 1.3.8 công phá Regex để chống mất tập cuối (Tập có chữ TL)
        ...
        // Kế thừa tính năng xếp nhóm ưu tiên Server của 1.3.8: Vietsub -> Vietsub 4K -> TM -> TM 4K
        servers.sort((a, b) => {
            const getScore = (name) => {
                let n = name.toLowerCase();
                let is4K = n.indexOf('4k') > -1;
                let isVietsub = n.indexOf('vietsub') > -1 || n.indexOf('mới cập nhật') > -1 || (!n.includes('thuyết minh') && !n.includes('thuyet minh'));
        ...
        if (!isPlayPage) {
            ...
            if (playBtnMatch) extra = playBtnMatch;
        }
...
function parseDetailResponse(html, url) {
    try {
        ...
        // Kế thừa cơ chế trích xuất link data-src an toàn không dùng _$ của bản 1.3.8
        ...
        if (allLink.length === 0) {
            ...
            while ((dMatch = dataRegex.exec(html)) !== null) {
                if (dMatch[1].indexOf('http') > -1) {
                    allLink.push({ link: dMatch[1], name: "Server Backup" });
                }
            }
        }
        ...
        allLink.forEach((item) => {
            ...
            if (n.indexOf('4k') > -1) pool.k4 = item.link;
            else if (n.indexOf('1080') > -1 || n.indexOf('hd') > -1) pool.hd = item.link;
            else if (l.indexOf('.m3u8') > -1 || l.indexOf('fbcdn') > -1) pool.anyM3u8 = item.link;
            else pool.anyEmbed = item.link;
        });
        ...
        // Loại bỏ hàm replace gây hỏng link fbcdn. Trả trực tiếp link nguyên bản mượt mà cho iOS
