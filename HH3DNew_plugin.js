...
var CONFIG = {
    ...
    icon: "https://bilutv.asia/img/bilutvlogo-ngang.jpg"
};

// Khởi tạo BASEURL tự động từ cấu hình
...
function parseMovieDetail(htmlContent, url) {
    try {
        ...
        var lurl = "", limg = "", lname = "Đang cập nhật...", ldes = "Không có mô tả.";
        var ldirec = "", lactor = "", lduran = "", status = "", category = "", episode_current = "", year = "";
        ...
        $child.find("a").each(function() {
            ...
            $parent.find(idserver).find("a").each(function() {
                ...
                var item4k = { id: this.attr("href") + "?type=4k", name: name, slug: "tap-" + name.replace(/\s/, "-") };
                items4k.push(item4k);
            });
        ...
        servers.forEach(server => {
            if (server.episodes && Array.isArray(server.episodes)) {
                server.episodes.sort((a, b) => getEpisodeNumber(a.name) - getEpisodeNumber(b.name));
            }
        });
...
function parseDetailResponse(html, url) {
    try {
        ...
        allLink.forEach((item) => {
            if (item.name.match(/4k/i) && item.link.endsWith('.m3u8')) pool.k4 = item.link;
            else if (item.name.match(/1080/i) && item.link.endsWith('.m3u8')) pool.hd = item.link;
            else if (item.link.endsWith('.m3u8')) pool.anyM3u8 = item.link;
            else if (item.link.includes('abyss')) pool.anyEmbed = item.link;
        });
