...
function getLinkHTML(inputContent, domain) {
    if (!domain) {
        domain = 'https://cdn.hdvideo.homes';
    }
    ...
    if (match) {
        ...
        while (c--) { 
            var key = e(c); 
            dict[key] = k[c] !== '' ? k[c] : key; 
        }
        code = p.replace(/\b\w+\b/g, function(token) { 
