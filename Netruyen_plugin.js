...
function parseMovieDetail(html, url) {
    ...
    try {
        ...
        var endchaper = $doc.find("#chapter_list li:first a").attr("href").replace(/.*\-(\d+)/, "$1");
        var maxchapter = Number(endchaper) + 1;
        ...
        for (var $j = 1; $j < maxchapter; $j++) {
            var startchapter = $j;
            var domain = `https://manga.alokillgtv.workers.dev/?target=nettruyen&namechapter=${namechapter}&maxchapter=${maxchapter}&slug=${slug}&startchapter=${startchapter}&iframe=true`;
            episodes.push({
                id: domain,
                name: "Chương " + $j,
                slug: "chuong-" + $j
