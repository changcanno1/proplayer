...
function getManifest() {
  return JSON.stringify({
    ...
    isEnabled: true,
    isAdult: false,
    type: "MOVIE", // Vẫn giữ MOVIE để tắt hiệu ứng ép zoom chậm chạp
    layoutType: "LIST",
...
function parseListResponse(html, apiUrl) {
  try {
    ...
    channels.forEach((channel) => {
      ...
      // Đẩy id trực tiếp để truyền thẳng vào parseDetailResponse (bỏ qua trang Chi tiết)
      ...
      items.push({
        ...
        // Thêm các cờ ép app phát ngay lập tức (Bypass Detail Page)
        directPlay: true,
        isPlayable: true,
        isLive: true 
      });
    ...
    console.error("⛔ [parseListResponse] ERROR: ", error);
...
// KHÔNG DÙNG parseMovieDetail nữa, gọi thẳng parseDetailResponse để Play
function parseDetailResponse(html, apiUrl) {
  try {
    ...
    console.error("⛔ [parseDetailResponse] ERROR: ", error);
    return "{}";
  }
}
...
// =============================================================================
// NHÓM 4: HELPERS (Giữ nguyên như cũ)
// =============================================================================
...
const GROUP_MAP = {
  vtv: "VTV ⭐", "thời sự": "VTV ⭐", vtvcab: "VTVcab 💎",
  htv: "HTV x HTVC 🧬", sctv: "SCTV 🎫", "địa phương": "Địa Phương 📺",
  "kênh địa phương": "Địa Phương 📺", thvl: "Địa Phương 📺",
  "dự phòng": "Backup 📌", "sự kiện vtvprime": "VTVPrime 🛰️",
  "thiết yếu": "VTV ⭐", "🌐| thiết yếu": "VTV ⭐", "htv/c": "HTV x HTVC 🧬",
  "asean huyndai cup 2026": "🔴 ASEAN HUYNDAI CUP 2026",
  "live events 🔴": "VOD 🎞️", "sự kiện fpt play": "FPTPlay 🏷️",
  radio: "Radio 📻", "phát thanh": "Radio 📻", "🇬🇧 uk radio": "Radio 📻",
  israel: "Israel 🌐", "🇰🇷| hàn quốc": "Hàn Quốc 🌐",
  "🇨🇳| trung quốc": "Trung Quốc 🌐", cctv: "Trung Quốc 🌐"
};

const CATEGORY_MAP = {
  vtv: "VTV ⭐", antvhd: "VTV ⭐", vietnamtoday: "VTV ⭐", qpvnhd: "VTV ⭐",
  vtvcab: "VTVcab 💎", on: "VTVcab 💎", vtvprime: "VTVPrime 🛰️",
  "htv-htvc": "HTV x HTVC 🧬", sctv: "SCTV 🎫", local: "Địa Phương 📺",
  backup: "Backup 📌", event: "🔴 ASEAN HUYNDAI CUP 2026", vod: "VOD 🎞️",
  fptplay: "FPTPlay 🏷️", radio: "Radio 📻", israel: "Israel 🌐",
  korea: "Hàn Quốc 🌐", china: "Trung Quốc 🌐"
};
...
function parseM3U(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.toUpperCase().includes("EXTINF:")) {
      currentChannel = {
        name: "No Name", tvgLogo: "", tvgGroup: "No Group", url: "", tvgId: "", channelId: count++, props: {}
      };
      ...
      if (currentChannel) {
        ...
        if (equalIdx !== -1) currentChannel.props[line.substring(10, equalIdx).trim()] = line.substring(equalIdx + 1).trim();
      }
    } else if (line.toUpperCase().startsWith("#EXTVLCOPT:")) {
      if (currentChannel) {
        const equalIdx = line.indexOf("=");
        if (equalIdx !== -1) currentChannel.props[line.substring(11, equalIdx).trim()] = line.substring(equalIdx + 1).trim();
      }
      ...
      if (currentChannel) {
        ...
        channels.push({ name: line.split("/").pop().toUpperCase() || "No Name", tvgLogo: "", tvgGroup: "No Group", url: line, channelId: count++, props: {} });
      }
    }
  }
  return channels;
...
function startsWithRegex(str, prefix) { return new RegExp("^" + prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).test(str); }
...
function atob(input) {
  ...
  let str = String(input).replace(/[\t\n\f\r ]/g, ""); let output = ""; let buffer = 0; let bits = 0;
...
function getClearKey(html, licenseKey) {
  ...
  try { 
    ...
    if (keyData.keys && Array.isArray(keyData.keys)) {
      keyData.keys.forEach((k) => { clearKey.drmKid = base64ToHex(k.kid); clearKey.drmKey = base64ToHex(k.k); });
    } else if (keyData.kid && keyData.k) { 
      clearKey.drmKid = base64ToHex(keyData.kid); clearKey.drmKey = base64ToHex(keyData.k);
    }
  } catch (error) { 
    if (licenseKey && licenseKey.includes(":") && licenseKey.split(":").length === 2) {
      const parts = licenseKey.split(":");
      if (/^[0-9a-fA-F]+$/.test(parts[0]) && /^[0-9a-fA-F]+$/.test(parts[1])) {
        clearKey.drmKid = parts[0].toLowerCase(); clearKey.drmKey = parts[1].toLowerCase();
      }
    } else {
