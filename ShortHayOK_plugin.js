...
// ===== HÀM MENU LIST BEGIN ======
{
  function getHomeSections() {
      return JSON.stringify([
          {"slug": BASEAPI + "/api/movies?type=4&pageSize=21&sort.sortBy=0&sort.isAsc=false","title": "Phim Mới","type": "Grid"}
      ]);
  }
...
function parseMovieDetail(html, url) {
    ...
    try {
        ...
        var description = movie.description.replace(/<[^>]*>|\&nbsp\;/g, '') || "";
        ...
        servers.push({
          name: "Gộp 1 Tập Full",
          episodes: [{
            id: url + "?join=true&maxepi=" + list.length + "&slug=" + encodeURI(movie.slug),
            name: "Gộp 1 Tập",
            slug: "tap-full"
          }]
        })
        ...
        var $return = JSON.stringify({
            ...
            lang: lang || "",
            extra: extra || "",
            datasend: BASE64.encode(JSON.stringify(m3u8file))
        });
...
// ===== HÀM TẠO XỬ LÝ STREAM PHIM BEGIN ======
{
  ...
  try {
    ...
    // Nếu kết quả trả về là JSON (Request tạo cache lần đầu)
